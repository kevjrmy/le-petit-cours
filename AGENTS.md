...
- **The footer draws on `/` and on the pages it points to, nowhere else**, and `Footer` decides
  that itself rather than the shell. It never links to the page you are reading: on `/` it points
  out, on a footer annexe it points back to « Accueil ». So a `where: "footer"` annexe is reachable
  from the home page only — **put a page anywhere else in the manifest if it has to be reachable
  from a lesson.**
