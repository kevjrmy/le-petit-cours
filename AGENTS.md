...
src/
  ...
  lib/                    site.ts, account.ts, supabase/client.ts — shuffle, adapter…
...
because an account holds no other identity (#22, #31). Signed out it offers « Se connecter » and
never reports « Non connecté »: reading without an account is the intended way to use the site, not
a fault to name. It learns who is signed in from `useAccount`, a **client** hook, so the layout above it never
